...
function performFavorableSwaps()
    ...
    local assignedMembers = filter( m -> m!=0, reshape(assignments,gCount*mPerG,1))
...
function getSortedGroups()
    ...
    for g=1:gCount
        tAssignments[g,:] = sort(tAssignments[g,:], by=x-> x==0 ? mCount+1 : x)
    end
    tAssignments = tAssignments[sortperm(assignments[:, 1]), :]
    return tAssignments
end
#-- end of sort Groups
...
function averagePrefOfGroup(g, withM=0, withoutM=0)
    ...
    local tAssignment = assignments[g,:]
    tAssignment = filter( m -> m!=0 && m!=withoutM, tAssignment)
...
function computeTheoriticalMaxScore()
    ...
    for m=1:mCount
        tPrefs[m,:] = sort(tPrefs[m,:], rev=true)
    end
