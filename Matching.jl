...
function performFavorableSwaps()
    ...
    while switches>0 
        ...
        #    rejectedBy[m] = Array{Int64,1}(undef,0)
        #end
        local tAssignedMembers = deepcopy(assignedMembers)
        switches = 0
        stable = true
        bene = true
        ...
        #for m in assignedMembers
            for m2 in assignedMembers
                ...
                #if (!searchStable && gScore + g2Score < g2ScoreWithM + gScoreWithM2) || (searchStable && (mPrefForG < mPrefForG2 && gPrefForM2<gPrefForM))  
...
function computeTheoriticalMaxScore()
    global mCount, gCount, assignments, prefs
    local tPrefs = zeros(Int64,mCount, mCount)
    for m=1:mCount
...
#-- Takes a really long time. Only run for testing on small sets 
#-- Basically does a full space scan
function computePracticalMaxScore(pAssignments=[], assignLevel = 0, members = [])
    global mCount, gCount, assignments, prefs, maxPScore, maxPSets

    if (assignLevel==0) 
        # first run
        members = 1:mCount
        pAssignments = zeros(Int64,gCount, mPerG)
    end
    local tAssignments = deepcopy(pAssignments)
    local tMaxPScore = -1000
    for setTmembers in combinations(members, mPerG)
        tAssignments[assignLevel+1,:] = setTmembers
        local restOfMembers = setdiff(members,setTmembers)
        if length(restOfMembers)==0
            # this is the end. 
            assignments = tAssignments
            (pScore,b) = averagePrefOfGroups()
            if (pScore>maxPScore)
                maxPScore = pScore
                tMaxPScore = pScore
                maxPSets[1]=tAssignments
                println("Score: $pScore")
            end
            return pScore
        end
        pScore = computePracticalMaxScore(tAssignments,assignLevel+1,restOfMembers)
        (pScore>tMaxPScore) && (tMaxPScore=pScore) 
    end
    return tMaxPScore
end
#-- End compute practical max score

#-- Read the data from File
function loadPreferenceData()
    global config, prefs, mCount, assignments, thMaxScore, maxPScore, maxPSets
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    # Stable Room mate
    global configFile = "config/ma-config-m6-g3-SR-1.yaml"
    ...
    #global configFile = "config/ma-config-m32-g8-GEN-1.yaml"
