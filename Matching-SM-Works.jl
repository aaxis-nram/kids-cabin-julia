...
function performFavorableSwaps()
    ...
    assignedMembers = filter( m -> m!=0, reshape(assignments,gCount*mPerG,1))
    ...
    while switches>0
        cycleCount += 1
        @info "Beginning Cycle $cycleCount"
        @show assignments
        switches = 0
        stable = true
        bene = true
        for m in assignedMembers
            for m2 in assignedMembers
                ...
                #if (gScore + g2Score < g2ScoreWithM + gScoreWithM2) || (mPrefForG < mPrefForG2 && gPrefForM2<gPrefForM)  
                #if (gScore<=gScoreWithM2 && g2Score<=g2ScoreWithM) || (gScore<=gScoreWithM2 && g2Score<g2ScoreWithM)
        ...
        println("Cycle count: $cycleCount Switches = $switches Stable = $stable / $bene")
    end
end
...
function printAssignments() 
    ...
    local tAssignments = assignments
    for g=1:gCount
        tAssignments[g,:] = sort(tAssignments[g,:])
    end
...
function sortGroups()
    global mCount, gCount, assignments
    for g=1:gCount
        assignments[g,:] = sort(assignments[g,:])
    end
    assignments = assignments[sortperm(assignments[:, 1]), :]
end

#-- Read the data from File
function loadPreferenceData()
    global config, prefs, mCount, assignments
